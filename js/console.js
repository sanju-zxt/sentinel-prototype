...
SEN.Console = (() => {
  ...
  function renderVoice(alert, opts = {}) {
    const prevText = voiceBanner.textContent;
    voiceBanner.classList.remove('hidden', 'crit');
    if (alert.cue === 'crit') voiceBanner.classList.add('crit');
    voiceBanner.innerHTML = `${alert.msg}<span class="who">${opts.who || ''} · ${alert.side.toUpperCase()} · ${alert.cue === 'crit' ? 'VOICE' : 'HAPTIC'}</span>`;
    heartbeat(alert);
  }
  ...
  function renderHealth(metrics) {
    ...
    const c = $('health-plot'); const g = c.getContext('2d');
    c.width = c.offsetWidth || 320; c.height = c.offsetHeight || 130;
