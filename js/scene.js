...
SEN.Scene = (() => {
  ...
  function clearDemo() {
    // remove only entities the demo runner spawned — keep the base street + user
    for (let i = entities.length - 1; i >= 0; i--) {
      if (entities[i].demoOnly) entities.splice(i, 1);
    }
  }
  ...
  function tick(dt, t) {
    for (const e of entities) {
      ...
      if (e.type === 'car') {
        ...
        // crowd separation (squared-distance gate first — avoids a sqrt per pair)
        for (const o of entities) {
          if (o === e || o.type !== 'pedestrian') continue;
          const dx = e.x - o.x, dy = e.y - o.y;
          const d2 = dx * dx + dy * dy;
          if (d2 >= 34 * 34 || d2 === 0) continue;
  ...
  const objectMemory = new Map();   // Memory pillar is the sole writer (no fabricated "when")
  ...
  return { entities, make, clear, clearDemo, reset, tick, buildStreet,
           get user(){return user}, set user(v){user=v},
           placeName, objectMemory };
})();