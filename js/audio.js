...
window.SentAudio = (() => {
  ...
  // browsers need a user gesture before the AudioContext starts; unlock on the
  // first key/pointer so the first alert is never silently dropped
  if (typeof document !== 'undefined' && document.addEventListener) {
    const unlock = () => { try { ensure(); } catch (e) { /* ignore */ } };
    document.addEventListener('pointerdown', unlock);
    document.addEventListener('keydown', unlock);
  }

  // older Safari lacks createStereoPanner — degrade to a plain gain (no pan)
  function makePan(c) {
    try { return c.createStereoPanner(); }
    catch (e) {
