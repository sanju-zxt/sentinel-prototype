...
window.SEN.Detector = (() => {
   ...
   * Output is column-major (row j, column i) = data[j * 8400 + i]; we scan it
   * in place with zero transposes, so no 8400-row buffers are ever allocated.
   */
  function postprocess(outputTensor, origW, origH) {
    const data = outputTensor.data;   // Float32Array [1, 84, 8400]
    ...
    for (let i = 0; i < N; i++) {
      ...
      for (let c = 0; c < numClasses; c++) {
        const s = data[classBase + c * N + i];
        if (s > maxScore) { maxScore = s; maxClass = c; }
      }
      if (maxScore < CONF_THRESHOLD) continue;

      // Convert from center format to corner format (model space → source pixels)
    ...
    // Sort by score descending, then suppress duplicates
