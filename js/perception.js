...
SEN.Perception = (() => {
  ...
  function reset() {
    ...
    lastSpoken = -99; lastRiver = -99; state = null;
  }
  ...
  function timeToImpact(u, e) {
    ...
    return tt > 0 ? tt : Infinity;   // negative root = already past you; not an imminent impact
  }
  ...
  function runReal(detections, u, opts = {}) {
    ...
    for (let i = 0; i < detections.length; i++) {
      ...
      if (type === 'pedestrian') {
        ...
        out.push({
          ...common, type, heading: az + Math.PI, speed: opts.assumeWalkSpeed !== false ? 1.2 * M_PX : 0, radius: 12,
          posture: ar >= 1.75 ? 'standing' : ar >= 1.25 ? 'leaning' : 'sitting',
        });
      } else if (type === 'car' || type === 'cyclist') {
        out.push({ ...common, type, heading: az + Math.PI, speed: opts.assumeVehicleSpeed !== false ? 4.5 * M_PX : 0, radius: 18 });
      } else {
