...
SEN.Localize = (() => {
  ...
  function updateOdometry(u, dt) {
    ...
    // odometry drift grows with distance walked; a small time-based noise grows
    // with *elapsed time* (dt), so σ never balloons while standing still
    belief.sigma = Math.min(MAX_SIGMA, belief.sigma + travelled * DRIFT_PER_M + 0.6 * dt);
  }
