...
SEN.Health = (() => {
  ...
  function update(dt, t) {
    ...
    // triggers
    if (simState.event === 'fever') { hr = clamp((hr + 118) / 2, 95, 121); }
  ...
  function fire(type, text, urg) {
    ...
    bus('health', { type, text });
  }

  function push(type, text, urg) { history.unshift({ type, text, urg, at: stamp() }); }
  ...
  return {
    ...
    push, fire, setEvent(__e) { simState.event = __e; simState.eventT = simState.t; if (__e === 'fatigue' && !lastAlert) { lastAlert = 'reset'; } },
