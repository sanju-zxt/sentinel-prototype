...
SEN.Social = (() => {
  ...
  function bodyWhisper(detected) {
    ...
    if (!p || p.arms !== 'open') return null;   // only the arms:'open' case is produced (demo 8)
    return {
  ...
  function doubleTap() {
    if (pilot.state === 'live' || pilot.state === 'connecting') return hangUp();
  ...
  function pilotUpdate(dt, specials) {
    if (pilot.state === 'connecting') {
      ...
      if (pilot.liveSince > 60) hangUp();   // flight cap — the call never runs forever
    }
  }
