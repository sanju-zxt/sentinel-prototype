...
SEN.Main = (() => {
  ...
  const CAM_HEAT = document.createElement('canvas');   // 160×160 depth pool (ImageData → 1 drawImage)
  const CAM_HEAT_CTX = CAM_HEAT.getContext('2d');
  let heatMap = null;
  ...
  async function realSense(dt) {
    if (!camOn) return;
    const img = SEN.Camera.snapshot(CAM_INPUT);
    if (!img) return;
    const detections = await SEN.Detector.detect(img);
    ...
    if (depthOn) {
      ...
      opts.distances = lastDists = await SEN.Depth.measure(img, detections);
  ...
  function drawPip(dets) {
    ...
    // Sentinel knows distance. teal = near → magenta = far. Rendered by
    // downsizing the 640×640 map into one 160×160 ImageData + a single
    // upscaling drawImage (was ~25k fillRects per frame).
    if (depthOn && SEN.Depth.map()) {
      const dm = SEN.Depth.map();           // 640×640 pseudo-range, larger = farther
      const CELL = 4;                        // sample a 4×4 block per pixel
      const COLS = 640 / CELL;               // 160
      if (!heatMap || heatMap.length !== COLS * COLS * 4) {
        CAM_HEAT.width = COLS; CAM_HEAT.height = COLS;
        heatMap = CAM_HEAT_CTX.createImageData(COLS, COLS);
      }
      const px = heatMap.data;
      for (let by = 0, i = 0; by < 640; by += CELL) {
        for (let bx = 0, row = by * 640; bx < 640; bx += CELL) {
          let acc = 0;
          for (let yy = 0; yy < CELL; yy++) {
            const r = row + yy * 640 + bx;
            for (let xx = 0; xx < CELL; xx++) acc += dm[r + xx];
          }
      ...
      CAM_HEAT_CTX.putImageData(heatMap, 0, 0);
      ctx.save();
      ctx.globalAlpha = 0.5;
      ctx.imageSmoothingEnabled = true;
      ctx.drawImage(CAM_HEAT, 0, 0, c.width, c.height);
      ctx.restore();
    }
  ...
  function handleHotkey(e) {
    ...
    switch (k) {
      ...
      case '0': startDemo(0); break;        // 0 = AUTO TOUR (Sentinel walks itself)
      case 'n': toggleGuide(true); break;   // N = turn-by-turn guide (press N again to stop)
      default: break;
    }
  }
  ...
  function loop(now) {
    ...
    const u = S().user;
    // the AUTO TOUR owns the user's legs — don't fight it with keyboard steering
    if (SEN.Demo.state.name !== 0) stepUser(u, dt);
    S().tick(dt, t);
    SEN.Demo.update(dt);
    ...
    if (camOn && detectorInitiated && now - lastDetect > 180) {
      lastDetect = now;
      realSense(dt).catch(() => {});   // fire-and-forget: detect/measure already swallow model errors
    }
    ...
    else if (!camOn) C().setHud({ plane: 'CAMERA+ULTRASOUND' });   // camera on → setHudSensor() owns the badge
    ...
    const specials = [
      ...
      ...SEN.Guide.update(dt, u, t),       // turn-by-turn nav findings (collapsed to one cadence)
  ...
  /* ---------- voice input (Web Speech) + turn-by-turn guide ---------- */
  function wireVoice() {
    const btn = document.getElementById('voice-toggle');
    if (!btn) return;                 // no voice button in this build
    SEN.Voice.init(btn, handleVoiceCommand);
  }

  // SpeechRecognizer → the same hotkey routes; anything unrecognized falls
  // back to the memory palace (so "where are my keys" always works).
  function handleVoiceCommand(text) {
    const q = String(text || '').toLowerCase().trim();
    const demo = /demo\s+(\d)/.exec(q);
    if (demo) { startDemo(Math.max(0, Math.min(9, +demo[1]))); return; }
    if (/\b(tour|intro|help)\b/.test(q)) { startDemo(0); return; }
    if (/\b(guide|navigate|navigation|turn by turn)\b/.test(q)) { toggleGuide(); return; }
    if (/\b(camera|video)\b/.test(q)) { toggleCamera(); return; }
    const a = SEN.Memory.ask(q || 'where are my keys');
    voice(a, 'hint', { who: 'VOICE COMMAND' });
    setStory(`<div class="h1">VOICE</div><div class="p">${a}</div>`);
  }

  function wireGuide() {
    const btn = document.getElementById('guide-toggle');
    if (!btn) return;                 // guide is run by hotkey N / voice in this build
    btn.addEventListener('click', () => toggleGuide());
  }

  function toggleGuide(force) {
    const res = force === false ? SEN.Guide.stop() : SEN.Guide.toggle();
    voice(res.spoken, res.active ? 'hint' : 'warn', { who: 'TURN-BY-TURN GUIDE' });
    setStory(`<div class="h1">NAVIGATION${res.active ? ' · ' + res.label : ''}</div><div class="p">${res.spoken}</div>`);
  }
  ...
  function wireEvents() {
    ...
    // health events (cardiac / fatigue) reach the same voice route — never a silent bus
    SEN.Health.on((evt, d) => {
      if (!d || !d.text) return;
      voice(d.text, d.urg === 'crit' ? 'crit' : 'warn', { who: 'HEALTH' });
    });
  }
