...
SEN.Memory = (() => {
  ...
  let seeded = false;
  function seedPalace() {
    if (seeded) return;              // boot + demo 7 both seed — never push duplicates
    seeded = true;
  ...
  function stash(objKey, x, y, tOverride) {
    ...
    drops.push(rec);
    if (drops.length > 40) drops.shift();   // bounded — objectMemory holds the canonical record per object
  ...
  function faceScan(near) {
    ...
    if (!rec) return null;
    const u = S().user;
    const inFront = distToFacing(u, fam) > 0.6;   // dot-product: only a head-on approach counts
